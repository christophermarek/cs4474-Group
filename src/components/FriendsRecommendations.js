...
export const FriendsRecomendations = ({
  currModal,
  setCurrModal,
  setFriendsReccomendations,
}) => {
    ...
    <>
      {currModal === "options" && selSongInfo && (
        <>
          <Options
            ...
            album_picture={selSongInfo.pic}
            song_name={selSongInfo.name}
            song_info={selSongInfo.info}
          />
        </>
      )}
      <div className={"recommendations " + (currModal !== "" ? "blur" : "")}>
        ...
        <div className={"friend-header lib-greeting "}>
          <motion.div
            onClick={() => setFriendsReccomendations(false)}
            whileTap={{ scale: 0.9 }}
          >
            <svg
              width="11px"
